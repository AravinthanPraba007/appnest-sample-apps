...
- **All I/O via AppNest SDK:** Use `$db` for persistence, `$fetch` for outbound HTTP, `$file` for files, `$next` for invoking other functions, `$schedule` for scheduled jobs. Do **not** use axios/fetch or raw DB clients.
