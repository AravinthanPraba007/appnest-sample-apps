...
- **Do not add `@aravinthan_p/appnest-app-sdk-utils` (or `appnest-sdk-utils`) to `app-backend/package.json` dependencies.** The AppNest SDK is provided by the platform at runtime. List only app-specific dependencies (e.g. `mssql`, other third-party libraries).
