...
**For AI (Cursor, etc.):** When generating or editing **any frontend code** (`app-frontend/`, `*.jsx`, `*.tsx`, or UI under this app), **read and apply this file first**. These rules are mandatory; do not generate frontend code that violates them.
...
| Rule | Constraint |
|------|------------|
| **API** | **MUST** call the backend via **`window.appnestClient.backend.invoke({ functionName, payload })`**. |
| **Parameter name** | Use **`functionName`** (exact). The value must match a function **exported** from `app-backend/server.js` and declared in `manifest.json` → `backend_api_functions`. |
...
| **MUST NOT** | Use `fetch`, `axios`, or any other HTTP client to call app backend. Do not use a different parameter name (e.g. `apiFunctionName`) for the invoke call in app frontend—use `functionName`. |
...
## 13. API handling and error handling

- **Separate API/backend logic from UI:** Use **services** or dedicated modules for `window.appnestClient.backend.invoke(...)`. Components should call a service/hook that returns data or error state, not invoke directly in the middle of JSX when it can be avoided.
...
## 14. Reusability and documentation
...
## 16. Before considering frontend complete
...
- Confirm: (a) Twigs packages in package.json with version `"*"`, (b) no raw HTML for controls, (c) layout uses Stack/Box, (d) backend calls use `window.appnestClient.backend.invoke({ functionName, payload })` and response `{ statusCode, body }`, (e) UI is responsive and polished for a SaaS context, (f) structure, naming, API/error handling, and accessibility (sections 5–15) are followed where applicable.
...
| Backend call | `window.appnestClient.backend.invoke({ functionName, payload })`; handle `{ statusCode, body }` | fetch/axios to app backend; wrong param name |
...
| UX | Responsive; mobile-first; SaaS "wow"; clean spacing, minimal layout | Fixed widths; cluttered layouts; inconsistent or unpolished UI |
