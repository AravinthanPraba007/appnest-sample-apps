...
| F7 | For each screen: **how does the user reach it** (default view, link from X, after action)? **Layout/sections** (e.g. header, table, form)? **User actions** (buttons, links) and which **backend functionName** each action calls (from backend_api_functions)? **Data shown** (source, e.g. response of `getMappings`)? **Empty / loading / error** states? | 11-ui-screens per-screen spec; tie actions to 08-api-contracts. |
